...
public class MySqlAccess {
		...
		try{
			...
			if (table == "exit"){
				...
				case "BasicInfo":
					...
					System.out.println("Enter the new date of birth in format yyyy-mm-dd");
					String DOB = keyboard.next();
					System.out.println("Enter the new date of impulse in format yyyy-mm-dd");
					String impulse = keyboard.next();
					query = "insert into TBI." + table + " (AID, DOB, Impulse) values ("
							+ Integer.toString(AID) + ",'" + DOB + "','" + impulse + "');";
					break;
				
				case "Weight":
					System.out.println("Enter the new animal number");
					AID = keyboard.nextInt();
					System.out.println("Enter the new date of weight in format yyyy-mm-dd");
					String weightDate = keyboard.next();
					System.out.println("Enter the new weight");
					Double weight = keyboard.nextDouble();
					query = "insert into TBI." + table + " (AID, WeightDate, Weight) values ("
							+ Integer.toString(AID) + ",'" + weightDate + "'," + Double.toString(weight) + ");";
					break;
					
				case "GNG":
					System.out.println("Enter the new animal number");
					AID = keyboard.nextInt();
					System.out.println("Enter the new date of test in format yyyy-mm-dd");
					String testDate = keyboard.next();
					System.out.println("Enter the correctness");
					Double correctness = keyboard.nextDouble();
					query = "insert into TBI." + table + " (AID, TestDate, Correctness) values ("
							+ Integer.toString(AID) + ",'" + testDate + "'," + Double.toString(correctness) + ");";
					break;
				}
			}
	...
	public void updateDataBase(Scanner keyboard) throws Exception {
		try{
			...
			if (table == "exit"){
				...
				case "BasicInfo":
					resultSet = statement.executeQuery("select AID from TBI.BasicInfo");
					printAID(resultSet);
					...
					case 1:
						System.out.println("Enter the new date of birth in format yyyy-mm-dd");
						DOB = keyboard.next();
						query = "update TBI." + table + " set DOB = '" + DOB + "' where AID = "
					...
					case 2:
						...
						query += "update TBI." + table + " set Impulse = '" + impulse + "' where AID = "
								+ Integer.toString(AID) + ";";
						break;
					...
					case 3:
						...
						query = "update TBI." + table + " set DOB = '" + DOB + "' where AID = "
								+ Integer.toString(AID) + ";";
						query1 = "update TBI." + table + " set Impulse = '" + impulse + "' where AID = "
								+ Integer.toString(AID) + ";";
					}
				...
				case "Weight":
					resultSet = statement.executeQuery("select distinct(AID) from TBI.Weight");
					System.out.println("Choose the animal number to update");
					printAID(resultSet);
					AID = keyboard.nextInt();
					
					resultSet = statement.executeQuery("select distinct(WeightDate) from TBI.Weight where AID = " + AID + ";");
					System.out.println("Choose the date of weight in format yyyy-mm-dd to update");
					...
					query = "update TBI." + table + " set Weight = "
							+ Double.toString(weight) + " where weightDate = '" + weightDate + "' and "
							+ "AID = " + Integer.toString(AID) + ";";
					break;
					
				case "GNG":
					resultSet = statement.executeQuery("select distinct(AID) from TBI.GNG");
					System.out.println("Choose the animal number to update");
					printAID(resultSet);
					AID = keyboard.nextInt();
					
					resultSet = statement.executeQuery("select distinct(TestDate) from TBI.GNG where AID = " + AID + ";");
					System.out.println("Choose the date of test in format yyyy-mm-dd to update");
					...
					query = "update TBI." + table + " set Correctness = "
							+ Double.toString(correctness) + " where TestDate = '" + testDate + "' and "
							+ "AID = " + Integer.toString(AID) + ";";
					break;
				}
			}
	...
	public void maxCorrectness(Scanner keyboard) throws Exception {
		try{
			...
			String query = "select TestDate, max(Correctness) as Max_Correctness from TBI.GNG where AID = " + AID;
			resultSet = statement.executeQuery(query);
			
			while (resultSet.next()){
				String maxCt = resultSet.getString("Max_Correctness");
				String testDate = resultSet.getString("TestDate");
				System.out.println("For " + AID + ", the maximal correctness is " 
						+ maxCt + " at " + testDate + ".");
			}
	...
	public void deleteLongDuration(Scanner keyboard) throws Exception {
		try{
			...
			String query = "delete from TBI.BalanceBeam where duration > " + timeLimit;
			Boolean result = statement.execute(query);
			if (!result){
	...
	private String chooseTable(Scanner keyboard){
		...
		while(flag){
			...
			switch (choice){
				case 1: System.out.println("You choose table Basic Infomation");
						table = "BasicInfo";
						flag = false;
						break;
				case 2: System.out.println("You choose table Weight.");
						table = "Weight";
						flag = false;
						break;
				case 3: System.out.println("You choose table Go/No Go experiment.");
						table = "GNG";
						flag = false;
						break;

				default: System.out.println("Invalid input. Please choose again.");
						choice = keyboard.nextInt();
						break;
			}
	...
	private void printDate(ResultSet resultSet, String dateType) throws SQLException{
